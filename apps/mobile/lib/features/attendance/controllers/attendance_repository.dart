...
class AttendanceRepository {
  ...
  Future<AttendanceSessionModel> clockOut(String locationId, String sessionId) async {
    final response = await apiClient.dio.post(
  ...
  Future<AttendanceSessionModel?> getActiveSession(String locationId) async {
    final response = await apiClient.dio.get('/locations/$locationId/attendance/active-session');
    if (response.data['session'] == null) return null;
    return AttendanceSessionModel.fromJson(response.data['session']);
  }

  Future<List<AttendanceSessionModel>> getSessions(String locationId, String period) async {
    final response = await apiClient.dio.get(
      '/locations/$locationId/attendance',
      queryParameters: {'period': period},
    );
    final data = response.data['data'] as List? ?? [];
    return data.map((e) => AttendanceSessionModel.fromJson(e as Map<String, dynamic>)).toList();
  }
}
