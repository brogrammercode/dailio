...
class SubscriptionPlansPage extends StatelessWidget {
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Expanded(
                              child: _buildStatBox(
                                  'Active Tiers', '04', 'All branches', null)),
                          const SizedBox(width: 8),
                          Expanded(
                              child: _buildStatBox(
                                  'Enrolled', '352', '+14% MoM', Colors.green)),
                          const SizedBox(width: 8),
                          Expanded(
                              child: _buildStatBox('Annual Run-rate', '₹14.2L',
                                  'Projected', null)),
                        ],
                      )
                    ],
                    ...
                    children: [
                              ...
                              children: [
                                const Text('Active & Publicly Available',
                                    style: TextStyle(
                                        ...
                                        fontWeight: FontWeight.bold)),
                                Text(
                                    'Members can purchase online and on-counter',
                                    style: TextStyle(
                                        fontSize: 10,
                              ...
                              child: Column(
                                ...
                                const Text('Currency',
                                    style: TextStyle(
                                        ...
                                        fontWeight: FontWeight.bold)),
                                const SizedBox(height: 8),
                                _buildDropdown('INR (₹)', icon: Iconsax.lock)
                              ])),
                        ],
                      ),
                      const SizedBox(height: 16),
                      const Text('Billing Frequency',
                          style: TextStyle(
                              fontSize: 11, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 8),
                      _buildDropdown('Every 12 Months (Annual)',
                          icon: Iconsax.arrow_down_1, isRightIcon: true),
                      const SizedBox(height: 16),
                      const Text('One-time Joining Fee',
                          style: TextStyle(
                              fontSize: 11, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 8),
                      Row(
                        ...
                        children: [
                          Expanded(
                              child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                const Text('Validity Duration',
                                    style: TextStyle(
                                        fontSize: 11,
                                        fontWeight: FontWeight.bold)),
                                const SizedBox(height: 8),
                                _buildTextField('365 Days')
                              ])),
                          const SizedBox(width: 12),
                          Expanded(
                              child: Column(
                                ...
                                const Text('Grace Period',
                                    style: TextStyle(
                                        ...
                                        fontWeight: FontWeight.bold)),
                                const SizedBox(height: 8),
                                _buildTextField('7 Days')
                              ])),
                        ],
                      ),
                      const SizedBox(height: 16),
                      const Text('Permitted Time Window',
                          style: TextStyle(
                              fontSize: 11, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 8),
                      _buildDropdown('Full Day Access (05:30 AM - 11:00 PM)',
                          icon: Iconsax.arrow_down_1, isRightIcon: true),
                      const SizedBox(height: 16),
                      const Text('Facility Entitlements',
                          style: TextStyle(
                              fontSize: 11, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 8),
                      _buildCheckboxRow(
                          'Main Branch - Indiranagar (Primary Home Base)',
                          true),
                      _buildCheckboxRow(
                          'All-branch Cross Attendance Allowed', true),
                      _buildCheckboxRow('Off-peak only restriction', false),
                      const SizedBox(height: 16),

                      // Perks
                      const Text('Complimentary Perks Included',
                          style: TextStyle(
                              fontSize: 11, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 8),
                      Wrap(
                        ...
                        children: [
                          ...
                          _buildPerkChip('1 Free Trainer Consultation',
                      ...
                      const Row(children: [
                        Icon(Iconsax.building_3,
                            size: 16, color: Colors.orange),
                        SizedBox(width: 8),
                        Text('Branch Availability Matrix',
                            style: TextStyle(
                                fontSize: 12, fontWeight: FontWeight.bold))
                      ]),
                            ...
                            Expanded(
                              child: RichText(
                                  text: const TextSpan(
                                    ...
                                    TextSpan(
                                        text:
                                            'Last updated by Organization Lead '),
                                    TextSpan(
                                        text: 'Marcus V.',
                                        style: TextStyle(
                                            fontWeight: FontWeight.bold)),
                                    TextSpan(
                                        text: ' on 15 Feb 2025, 14:22 IST'),
                                  ])),
                            )
                          ],
                        ),
                ...
                child: Column(
                  children: [
                    ElevatedButton.icon(
                      ...
                      label: const Text('Save Subscription Plan Changes',
  ...
  Widget _buildTab(String label, bool isActive) {
    return Container(
      ...
      child: Row(
        children: [
          if (isActive) ...[
            const Icon(Iconsax.verify, size: 14, color: Colors.white),
            const SizedBox(width: 8)
          ],
          Text(label,
              style: TextStyle(
  ...
  Widget _buildDropdown(String text,
      {required IconData icon, bool isRightIcon = false}) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
      decoration: BoxDecoration(
          color: Colors.grey.shade50,
  ...
  Widget _buildCheckboxRow(String label, bool isChecked) {
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(8),
          border: Border.all(
              color: isChecked ? Colors.blue.shade100 : Colors.grey.shade200)),
      child: Row(
        children: [
  ...
  Widget _buildBranchMatrixChip(String label, bool isActive) {
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(isActive ? Iconsax.verify : Icons.block,
